...
function comments(id){if(!requireMember())return;const post=posts().find(r=>r.id===id);openModal(esc(post?.name||'이야기 나누기'),`<div>${(state.comments[id]||[]).map(c=>`<div class="comment"><b>${esc(nickname())}</b><br>${esc(typeof c==='string'?c:c.text)}</div>`).join('')||'<p class="empty">첫 이야기를 남겨주세요.</p>'}</div><form id="comment-form"><p id="comment-error" class="form-error" role="alert"></p><label class="field">댓글<textarea name="comment" required maxlength="500" placeholder="이 찻자리에 한마디"></textarea></label><button class="primary">댓글 남기기</button></form>`);$('#comment-form').onsubmit=async e=>{e.preventDefault();if(!requireMember())return;const c=new FormData(e.target).get('comment').trim();if(!c)return;const next=draftState();next.comments[id]=[...(next.comments[id]||[]),{text:c,author:nickname(),authorUid:currentUser.uid}];if(await commitState(next)){comments(id);render();}}}
...
function renderAdmin(){if(!requireMember())return;$('#app').innerHTML=head('관리자 체험','커뮤니티 운영 화면을 미리 살펴보세요.')+`<div class="notice">내 계정에만 적용되는 운영 화면 체험입니다. 다른 회원의 데이터나 커뮤니티 전체에는 영향을 주지 않습니다.</div><h2 style="margin-bottom:15px">공지 관리</h2><form id="notice-form" class="list-item"><label class="field" style="flex:1;margin:0">공지 제목<input name="title" required maxlength="100" placeholder="새 소식을 입력해주세요"></label><button class="primary">공지 추가</button></form>${state.announcements.map((n,i)=>`<div class="list-item"><span>${esc(n)}</span><button class="danger" onclick="removeAnnouncement(${i})">삭제</button></div>`).join('')}<h2 style="margin:28px 0 15px">공개 기록 관리</h2>${[...state.records.filter(r=>r.visibility==='public')].map(r=>`<div class="list-item"><div><h3>${esc(r.name)}</h3><p>${esc(nickname())} · ${r.type}</p></div><button class="small-btn" onclick="moderate('${r.id}')">${state.hidden.includes(r.id)?'숨김 해제':'공개 피드에서 숨기기'}</button></div>`).join('')}`;$('#notice-form').onsubmit=async e=>{e.preventDefault();if(!requireMember())return;const t=new FormData(e.target).get('title').trim();if(!t)return;const next=draftState();next.announcements.push(t);if(await commitState(next)){renderAdmin();toast('공지를 저장했어요.');}}}
...
async function toggle(key,id){if(!requireMember()||!['likes','saved'].includes(key))return;const next=draftState();next[key]=next[key].includes(id)?next[key].filter(x=>x!==id):[...next[key],id];if(await commitState(next))renderFeed()}
async function moderate(id){if(!requireMember())return;const next=draftState();next.hidden=next.hidden.includes(id)?next.hidden.filter(x=>x!==id):[...next.hidden,id];if(await commitState(next))renderAdmin()}
async function removeAnnouncement(index){if(!requireMember())return;const next=draftState();next.announcements.splice(index,1);if(await commitState(next))renderAdmin()}
function updateAuthUI(){
 const a=globalThis.chagokAuth,user=a?.user||null;const changed=(currentUser?.uid||null)!==(user?.uid||null);
 if(changed){++cloudGeneration;closeModal();state=JSON.parse(JSON.stringify(defaults));cloud={uid:null,phase:'idle',revision:null,error:null};filter='전체';page='calendar';}
 currentUser=user;storageKey=user?'chagok-user-'+user.uid:'chagok-v1';
