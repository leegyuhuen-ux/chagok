...
function persist(){if(!requireMember())return;try{localStorage.setItem(storageKey,JSON.stringify(state));return true}catch{toast('저장 공간이 부족해요. 이번 변경은 화면에만 유지됩니다.');return false}}
...
function go(p){if(!requireMember())return;page=p;document.querySelectorAll('nav button').forEach(b=>b.classList.toggle('active',b.dataset.page===p));$('#breadcrumb').textContent='나의 찻자리 / '+({calendar:'내 차생활',feed:'차생활 둘러보기',talk:'이야기방',events:'이벤트',admin:'관리자 체험'}[p]);render()}
document.querySelectorAll('[data-page]').forEach(b=>b.addEventListener('click',()=>go(b.dataset.page)));$('.brand').onclick=()=>go('calendar');
function render(){if(!requireMember())return;document.body?.classList.remove('membership-locked');({calendar:renderCalendar,feed:renderFeed,talk:renderTalk,events:renderEvents,admin:renderAdmin}[page]||renderCalendar)();if(page!=='admin'&&state.announcements.length)$('#app').insertAdjacentHTML('afterbegin', '<div class="notice">공지 · '+state.announcements.map(esc).join(' · ')+'</div>')}
...
function renderAdmin(){if(!requireMember())return;$('#app').innerHTML=head('관리자 체험','커뮤니티 운영 화면을 미리 살펴보세요.')+`<div class="notice">로그인한 회원이 둘러볼 수 있는 로컬 체험 화면입니다. 변경은 이 브라우저에만 적용되며, 실제 관리자 인증은 아직 연결되지 않았습니다.</div><h2 style="margin-bottom:15px">공지 관리</h2><form id="notice-form" class="list-item"><label class="field" style="flex:1;margin:0">공지 제목<input name="title" required maxlength="100" placeholder="새 소식을 입력해주세요"></label><button class="primary">공지 추가</button></form>${state.announcements.map((n,i)=>`<div class="list-item"><span>${esc(n)}</span><button class="danger" onclick="state.announcements.splice(${i},1);persist();renderAdmin()">삭제</button></div>`).join('')}<h2 style="margin:28px 0 15px">공개 기록 관리</h2>${[...state.records.filter(r=>r.visibility==='public')].map(r=>`<div class="list-item"><div><h3>${esc(r.name)}</h3><p>${esc(nickname())} · ${r.type}</p></div><button class="small-btn" onclick="moderate('${r.id}')">${state.hidden.includes(r.id)?'숨김 해제':'공개 피드에서 숨기기'}</button></div>`).join('')}`;$('#notice-form').onsubmit=e=>{e.preventDefault();if(!requireMember())return;const t=new FormData(e.target).get('title').trim();if(!t)return;state.announcements.push(t);persist();renderAdmin();toast('공지를 추가했어요.')}}
function moderate(id){if(!requireMember())return;state.hidden=state.hidden.includes(id)?state.hidden.filter(x=>x!==id):[...state.hidden,id];persist();renderAdmin()}
...
function isMember(){const a=globalThis.chagokAuth;return !!(a?.ready&&!a.error&&currentUser&&a.user?.uid===currentUser.uid&&validNickname(nickname()))}
function requireMember(){if(isMember())return true;closeModal();renderGate();return false}
function renderGate(){
 ...
 if(currentUser&&a?.ready&&!a.error){renderNicknameSetup();return;}
...
async function saveNickname(value,uid){
 if(!currentUser||currentUser.uid!==uid||globalThis.chagokAuth?.user?.uid!==uid)return false;
 ...
 try{
  const result=await globalThis.chagokAuth.setNickname(value);
  if(currentUser?.uid!==uid||result.uid!==uid)return false;
  state.nickname=value;currentUser={...currentUser,displayName:value};
  const saved=persist();closeModal();updateAuthUI();
  if(saved)toast('닉네임을 저장했어요.');
  return true;
 }catch(error){if(currentUser?.uid===uid&&$('#nickname-error'))$('#nickname-error').textContent='닉네임을 저장하지 못했어요. 연결을 확인하고 다시 시도해주세요.';return false;}
 finally{if(currentUser?.uid===uid&&$('#save-nickname')){$('#save-nickname').disabled=false;$('#save-nickname').textContent='닉네임 저장';}}
}
function showProfile(){
 if(!requireMember())return;
 openModal('나의 찻자리',`<h3>${esc(nickname())}</h3><p class="sub">내 기록 ${state.records.length}개 · 저장한 기록 ${state.saved.length}개</p><div class="notice">차 기록은 현재 이 브라우저에만 저장돼요. 다른 이용자와의 공유와 기기 간 동기화는 준비 중이에요.</div><div class="form-actions"><button class="small-btn" onclick="renderNicknameSetup(true)">닉네임 변경</button><button class="small-btn" onclick="closeModal();filter='저장한 기록';go('feed')">저장한 기록</button><button class="primary" onclick="logoutGoogle()">로그아웃</button></div>`);
}
function updateAuthUI(){
 const a=globalThis.chagokAuth,user=a?.user||null;
 const nextKey=user?'chagok-user-'+user.uid:'chagok-v1';
 if(nextKey!==storageKey){
  closeModal();storageKey=nextKey;let saved;
  if(user){try{saved=JSON.parse(localStorage.getItem(storageKey))}catch{}}
  state=saved&&Array.isArray(saved.records)?{...JSON.parse(JSON.stringify(defaults)),...saved}:{...JSON.parse(JSON.stringify(defaults)),cleanStart:true};
  filter='전체';page='calendar';
 }
 currentUser=user;
 $('#login').textContent=user?'내 계정':a?.configured&&!a.ready?'로그인 확인 중…':'Google 로그인 ↗';
 $('#profile b').textContent=user?(nickname()||'닉네임 설정'):'로그인이 필요해요';
 $('#profile .avatar').textContent=(user&&nickname()?nickname():'茶').slice(0,1);
 $('#profile small').textContent=user?'나의 찻자리':'Google로 시작하기';
 document.body?.classList.toggle('membership-locked',!isMember());
 if(isMember())go(page);else renderGate();
}
