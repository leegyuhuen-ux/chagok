...
vm.runInContext(fs.readFileSync(require('path').join(__dirname,'../dist/community.js'),'utf8'),context);
...
context.chagokAuth={ready:true,configured:true,user:{uid:'alice'},getStore:async()=>store,getCommunity:async()=>({watch(uid,admin,cb){cb([]);return ()=>{}},watchNotices(uid,cb){cb([]);return ()=>{}}})};
...
context.chagokAuth.user.isAdmin=true;run('updateAuthUI();go("admin")');assert(el('#app').innerHTML.includes('커뮤니티 관리'));
context.chagokAuth.user=null;run('updateAuthUI()');assert.equal(run('community.items.length'),0);assert.equal(run('community.notices.length'),0);
